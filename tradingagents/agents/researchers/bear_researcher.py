...
"""
        ...
        # Query TradeSimilaritySearch for historical SELL trade performance
        historical_sell_context = ""
        try:
            ...
            if similar_trades.get("similar_trades"):
                ...
                historical_sell_context = f"""
HISTORICAL SELL/SHORT PERFORMANCE ({stats['sample_size']} similar trades):
- Win Rate: {stats['win_rate']*100:.0f}%
- Avg Risk-Reward: {stats['avg_rr']:.2f}R
- Best Trade: {stats['best_rr']:+.2f}R | Worst: {stats['worst_rr']:+.2f}R

{"Use this strong historical performance to support your bearish case." if stats['win_rate'] >= 0.55 else "Historical short win rate is below 55% - acknowledge this when presenting your bearish argument."}
"""
        except Exception:
...
Company fundamentals report: {fundamentals_report}{smc_instruction}{historical_sell_context}
